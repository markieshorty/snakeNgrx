  ...
  switch (action.type) {
    ...
    case GameActionTypes.MoveDown: {
      return {
        ...state,
        directionOfTravel:
          state.directionOfTravel !== Directions.UP
            ? Directions.DOWN
            : Directions.UP,
      };
    }
    case GameActionTypes.MoveUp: {
      return {
        ...state,
        directionOfTravel:
          state.directionOfTravel !== Directions.DOWN
            ? Directions.UP
            : Directions.DOWN,
      };
    }
    case GameActionTypes.MoveRight: {
      return {
        ...state,
        directionOfTravel:
          state.directionOfTravel !== Directions.LEFT
            ? Directions.RIGHT
            : Directions.LEFT,
      };
    }
    case GameActionTypes.MoveLeft: {
      return {
        ...state,
        directionOfTravel:
          state.directionOfTravel !== Directions.RIGHT
            ? Directions.LEFT
            : Directions.RIGHT,
      };
    }
    case GameActionTypes.AnimationFrame: {
      ...
      newSnakePosition.body = JSON.parse(
        JSON.stringify(state.snakePosition.body)
      );
      ...
      newSnakePosition = GameReducerHelper.updateBody(
        newSnakePosition,
        state,
        justAte
      );
