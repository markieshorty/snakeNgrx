...
export class MainComponent implements OnInit {
  ...
  ngOnInit() {
    this.store
      .pipe(select(fromGameSelectors.getSnakeHeadPosition))
      .subscribe((snakeHeadPosition) => {
        this.snakeHeadPosition = snakeHeadPosition;
      });

    this.store.pipe(select(fromGameSelectors.getApple)).subscribe((apple) => {
      this.apple = apple;
    });

    this.store.pipe(select(fromGameSelectors.getScore)).subscribe((score) => {
      this.score = score;
    });

    this.store
      .pipe(select(fromGameSelectors.getGameOver))
      .subscribe((gameOver) => {
    ...
    this.store
      .pipe(select(fromGameSelectors.getHighestScore))
      .subscribe((highestScore) => {
